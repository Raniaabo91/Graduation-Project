...
export default function Quizz() {
	  // En array med frågor och svarsalternativ
	...
	const questions = [
		...
		},
		{
			...
			answerOptions: [
				...
				// Fler frågor och svarsalternativ

	];

	// Använder useState-hook för att hålla reda på den aktuella frågan
	const [currentQuestion, setCurrentQuestion] = useState(0);
  // Använder useState-hook för att hålla reda på om resultatsidan ska visas
	const [showScore, setShowScore] = useState(false);
  // Använder useState-hook för att hålla reda på poängen
	const [score, setScore] = useState(0);

  // En funktion som kallas när användaren väljer ett svarsalternativ
	const handleAnswerOptionClick = (isCorrect) => {
    // Ökar poängen med 1 om svarsalternativet är korrekt
		 // Ökar poängen med 1 om svarsalternativet är korrekt
		 if (isCorrect) {
			setScore(score + 1);
		}

    // Räknar upp den aktuella frågan med 1
		const nextQuestion = currentQuestion + 1;
    // Kontrollerar om det finns fler frågor
		if (nextQuestion < questions.length) {
      // Uppdaterar den aktuella frågan om det finns fler frågor
			setCurrentQuestion(nextQuestion);
		} else {
      // Visar resultatsidan om det inte finns fler frågor
			setShowScore(true);
		}
	};
	return (
		<div className='quizz'>
			{showScore ? (
				<div className='score-section'>
					...
					<Link to="/Quiz1" className="btn-primary">
            Gör om quizz
          </Link>
		  					</div>
			) : (
